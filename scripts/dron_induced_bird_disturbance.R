    ...
    for (i in 1:length(target_birds)) {
        ...
        for (a in 1:length(altitude_list)) {
            ...
            while (xy_disp_m_i > 0) {
                ...
                xy_stop <- xy_vel_ms_i**2 / 2 * abs(max_xyz_acc_mss)
                if (z_disp_m_i < altitude_list[a]) {
                    # print("altitude below target")
                    if ((altitude_list[a] - z_disp_m_i) > z_stop) {
                        # print("outside stopping distance")
                        if (z_vel_ms_i < max_z_vel_ms) {
                            # print("ascending accelerating")
                            xyz_acc_mss_i <- max_xyz_acc_mss}
                        else {
                            # print("ascending constant")
                            xyz_acc_mss_i <- 0}}
                    else {
                        # print("ascending decelerating")
                        xyz_acc_mss_i <- -max_xyz_acc_mss}
                    z_vel_ms_i <- z_vel_ms_i + xyz_acc_mss_i * dt
                    z_disp_m_i <- z_disp_m_i + z_vel_ms_i * dt}
                else {
                    if (xy_disp_m_i > xy_stop) {
                        # print("distance further than stopping distance")
                        z_vel_ms_i <- 0
                        if (xy_vel_ms_i < max_xy_vel_ms) {
                            # print("advancing accelerating")
                            xyz_acc_mss_i <- max_xyz_acc_mss}
                        else {
                            # print("advancing constant")
                            xyz_acc_mss_i <- 0}}
                    else {
                        # print("advancing decelerating")
                        xyz_acc_mss_i <- -max_xyz_acc_mss}
                    xy_disp_m_i <- xy_disp_m_i - xy_vel_ms_i * dt
                    xy_vel_ms_i <- xy_vel_ms_i + xyz_acc_mss_i * dt}
                ...
                # print(paste0("altitude", z_disp_m_i))
                # print(paste0("distance", xy_disp_m_i))
                # print(paste0("acceleration", xyz_acc_mss_i))
            }
...
ggplot(data = advancing, aes(x = xy_disp_m, y = z_disp_m, z = surv_prob)) +
facet_wrap("common_name") +
geom_contour_filled()
