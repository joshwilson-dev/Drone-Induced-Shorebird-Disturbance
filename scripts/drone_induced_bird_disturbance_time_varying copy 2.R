...
prepare_data <- function(df, istrain) {
    data_clean <- df %>%
        ...
        mutate(
            ...
            presence_more_susceptible_species = as.factor(presence_more_susceptible_species),
...
data_ped_train_species <- data_ped_train %>%
    ...
    droplevels() %>%
    select(id, presence_more_susceptible_species, ped_status, tstart, tend, interval, offset, common_name, drone, location, flock_number, cloud_cover_p, wind_speed_ms, xy_disp_m, z_disp_m, xyz_acc_mss, month_aest)
...
fit <- gam(
    ...
    common_name +
    presence_more_susceptible_species +
...
new_dataframe <- data_ped_train_species %>%
    ungroup() %>%
    make_newdata(
        # xy_disp_m = seq_range(xy_disp_m, n = 20),
        # z_disp_m = seq_range(z_disp_m, n = 20),
        # month_aest = seq_range(month_aest, n = 10),
        # xyz_acc_mss = seq_range(xyz_acc_mss, n = 10)) %>%
        common_name = unique(common_name)) %>%
        # presence_more_susceptible_species = unique(presence_more_susceptible_species)) %>%
    add_term(fit, term = "common_name", exclude = c("s(flock_number)", "s(location)")) %>%
    rename(
        "common_name_fit" = fit,
        "common_name_ci_lower" = ci_lower,
        "common_name_ci_upper" = ci_upper)
    ...
    # add_term(fit, term = "month_aest", exclude = c("s(flock_number)", "s(location)")) %>%
    # rename(
    #     "month_aest_fit" = fit,
    #     "month_aest_ci_lower" = ci_lower,
    #     "month_aest_ci_upper" = ci_upper)
    # add_term(fit, term = "presence_more_susceptible_species", exclude = c("s(flock_number)", "s(location)")) %>%
    # rename(
    #     "presence_more_susceptible_species_fit" = fit,
    #     "presence_more_susceptible_species_ci_lower" = ci_lower,
    #     "presence_more_susceptible_species_ci_upper" = ci_upper)

trial <- new_dataframe
    # filter(common_name == "bar_tailed_godwit")
    # filter(presence_more_susceptible_species == FALSE) %>%
    # filter(xy_disp_m == median(xy_disp_m)) %>%
    # filter(z_disp_m == median(z_disp_m)) %>%
    # filter(month_aest == 5)
...
ggplot(data = trial, aes(x = month_aest, y = month_aest_fit)) +
geom_line() +
geom_ribbon(aes(ymin = month_aest_ci_lower, ymax = month_aest_ci_upper), alpha = 0.2)
...
ggplot(data = trial, aes(x = presence_more_susceptible_species, y = presence_more_susceptible_species_fit)) +
geom_pointrange(aes(ymin = presence_more_susceptible_species_ci_lower, ymax = presence_more_susceptible_species_ci_upper)) +
theme(axis.text.x = element_text(angle = 90, vjust = 0.5))

ggplot(data = trial, aes(x = common_name, y = common_name_fit)) +
geom_pointrange(aes(ymin = common_name_ci_lower, ymax = common_name_ci_upper)) +
theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
