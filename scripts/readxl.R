...
#### import data ####
  ...
  # Import Data
    importdata <- function (filenames) {
      ...
      # "R:/data/shorebird-disturbance/results/test-054/00/t054f00.xlsx"
      for (i in 1:length(filenames)) {
        filename =  filenames[i]
        test = substr(filename, 2,4)
        flight = substr(filename, 6,8)
        location = paste0("R:/data/shorebird-disturbance/results/test-", test, "/", flight, "/", filename,".xlsx")
        assign(filename, read_excel(location, col_types = "text"), envir=.GlobalEnv)
      }
    }
